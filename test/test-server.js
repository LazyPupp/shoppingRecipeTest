...
describe('recipesTest', function(){
  before(function(){return runServer();});
  after(function(){return closeServer();});
  it('should retrieve items on GET', function(){
    return chai.request(app)
    .get("/recipes")
    .then(response=>{
  ...
  it('should add new item on POST', function(){
    const newObj = {name:"vindaloo", ingredients:['tofu','lentils','spices']};
    return chai.request(app)
    .post("/recipes")
    .send(newObj)
    .then(res=>{
      ...
      res.body.should.deep.equal(Object.assign(newObj, {id: res.body.id}));
